...
DATABASE_URL = (
    ...
    or "sqlite:///" + os.path.abspath("app.db")
)
...
with tab1:
    ...
    st.caption("Füge zwei Spalten ein: **Wert** und **zensiert (0/1)**.")
    ...
    if not items:
        ...
        for it in items:
            with st.expander(f"#{it.id} – {it.original_name} • {it.uploaded_at}"):
                ...
                st.caption("Weibull Probability Plot")
                try:
                    beta = summary.get("shape")
                    eta = summary.get("scale")
                    if beta and eta:
                        xs = np.linspace(0.01, eta*3, 200)
                        cdf = 1 - np.exp(-(xs/eta)**beta)
